...
impl TryFrom<[i16; 3]> for Color {
  ...
  fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
    // let [r, g, b] = arr;
    // if arr.into_iter().all(is_valid_rgb) {
    //   Ok(Self {
    //     red: r as u8,
    //     green: g as u8,
    //     blue: b as u8,
...
impl TryFrom<&[i16]> for Color {
  ...
  fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
    // let mut x = slice.iter();
    // let (Some(&red), Some(&green), Some(&blue), None) = (x.next(), x.next(), x.next(), x.next())
    // else {
    //   return Err(IntoColorError::BadLen);
    // };
    // if [red, green, blue].into_iter().all(is_valid_rgb) {
    //   Ok(Self {
    //     red: red as u8,
    //     green: green as u8,
    //     blue: blue as u8,
